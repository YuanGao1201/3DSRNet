...
class Base_Model(nn.Module):
    ...
    def init_process(self, opt):
        ...
        if self.training:
            self.init_loss(opt)
            # apex init
            # self.netG, self.optimizer_G = amp.initialize(self.netG, self.optimizer_G, opt_level="O0")
            # self.netD, self.optimizer_D = amp.initialize(self.netD, self.optimizer_D, opt_level="O0")

            # [self.netG, self.netD], [self.optimizer_G, self.optimizer_D] = amp.initialize([self.netG, self.netD],
        ...
        # else:
            # self.netG = amp.initialize(self.netG, opt_level="O1")
            # self.netD = amp.initialize(self.netD, opt_level="O0")
