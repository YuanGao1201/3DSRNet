...
def define_3DG(noise_len, input_shape, output_shape, input_nc, output_nc, ngf, which_model_netG, n_downsampling,
               ...
               encoder_input_nc=1,
               encoder_norm='instance2d', encoder_blocks=3,
               skip_num=1, activation_type='relu', opt=None, transformer_flag=False, stl_flag=False, conformer_flag=False):
    netG = None
    ...
    elif which_model_netG == 'multiview_network_denseUNetFuse_transposed': # our
        from .generator.dense_generator_multiview import UNetLike_DownStep5, MultiView_UNetLike_DenseDimensionNet
        netG = MultiView_UNetLike_DenseDimensionNet(
            view1Model=UNetLike_DownStep5(input_shape=encoder_input_shape[0],
                                          encoder_input_channels=encoder_input_nc,
                                          # encoder_input_channels=64,  # our train 256
                                          decoder_output_channels=output_nc, decoder_out_activation=activation_layer,
                                          encoder_norm_layer=encoder_norm_layer, decoder_norm_layer=decoder_norm_layer,
                                          upsample_mode='transposed', decoder_feature_out=True),
            view2Model=UNetLike_DownStep5(input_shape=encoder_input_shape[0],
                                          encoder_input_channels=encoder_input_nc,
                                          # encoder_input_channels=64,    # our train 256
                                          decoder_output_channels=output_nc, decoder_out_activation=activation_layer,
                                          encoder_norm_layer=encoder_norm_layer, decoder_norm_layer=decoder_norm_layer,
                                          upsample_mode='transposed', decoder_feature_out=True),
            view1Order=opt.CTOrder_Xray1, view2Order=opt.CTOrder_Xray2,
            # backToSub=False,
            backToSub=True,
            decoder_output_channels=output_nc, decoder_out_activation=activation_layer,
            decoder_block_list=[1, 1, 1, 1, 1, 0], decoder_norm_layer=decoder_norm_layer, upsample_mode='transposed',
            transformer_flag=transformer_flag, stl_flag=stl_flag, conformer_flag=conformer_flag)
