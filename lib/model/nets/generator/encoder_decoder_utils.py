...
class Dense_3DBlock(nn.Module):
    def __init__(self, num_layers, num_input_features, up_sample=False, bn_size=4, growth_rate=16,
                 norm_layer=nn.BatchNorm3d, activation=nn.ReLU(True), use_bias=True):
        ...
        for i in range(num_layers):
            conv_block += [
                _DenseLayer3D(num_input_features + i * growth_rate, growth_rate, up_sample, bn_size, norm_layer,
                              activation, use_bias)]
        if up_sample:
...
class _DenseLayer2D(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, norm_layer, activation, use_bias):
        ...
        self.add_module('conv2', nn.Conv2d(bn_size * growth_rate, growth_rate,
                                           kernel_size=3, stride=1, padding=1, bias=use_bias)),
        # our light
        # # self.add_module('2_conv1_light', nn.Conv2d(bn_size * growth_rate, bn_size * growth_rate*3, kernel_size=1, stride=1, padding=0, bias=False)),
        # # self.add_module('2_bn1_light', nn.BatchNorm2d(bn_size * growth_rate*3)),
        # # self.add_module('2_Relu1_light', nn.ReLU(inplace=True)),
        # self.add_module('2_conv2_light', nn.Conv2d(bn_size * growth_rate, bn_size * growth_rate, kernel_size=3, stride=1, padding=3//2, groups=(bn_size * growth_rate), bias=False)),
        # self.add_module('2_bn2_light', nn.BatchNorm2d(bn_size * growth_rate)),
        # self.add_module('2_Relu2_light', nn.ReLU(inplace=True)),
        # self.add_module('2_conv3_light', nn.Conv2d(bn_size * growth_rate, growth_rate, kernel_size=1, stride=1, padding=0, bias=False)),
...
class _DenseBlock2D_Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, norm_layer=nn.BatchNorm2d, activation=nn.ReLU(True),
                 use_bias=True):
...
class DenseBlock2D_Transition(nn.Module):
    def __init__(self, num_input_features, num_output_features, norm_layer=nn.BatchNorm2d, activation=nn.ReLU(True),
        ...
        self.conv_block = _DenseBlock2D_Transition(num_input_features, num_output_features, norm_layer, activation,
                                                   use_bias)
...
'''our'''
# class Transposed_And_Add_View3(nn.Module):
#     def __init__(self, view1Order, view2Order, view3Order, sortOrder=None):
#         super(Transposed_And_Add_View3, self).__init__()
#         # self.view1Order = view1Order
#         # self.view2Order = view2Order
#         self.permuteView1 = tuple(np.argsort(view1Order))
#         self.permuteView2 = tuple(np.argsort(view2Order))
...
#     def forward(self, *input):
#         # return tensor in order of sortOrder
#         return (input[0].permute(*self.permuteView1) + input[1].permute(*self.permuteView2) + input[2].permute(
#             *self.permuteView3))
