...
class CTGAN(Base_Model):
    ...
    def init_network(self, opt):
        ...
        # self.visual_names = ['G_real', 'G_fake', 'G_input', 'G_Map_fake_F', 'G_Map_real_F', 'G_Map_fake_S',
        #                      'G_Map_real_S']
        self.visual_names = ['G_real', 'G_fake', 'G_input']
    ...
    def forward(self):
        ...
        if self.conditional_D and self.training:
            self.G_condition_D = self.dimension_order_std(self.G_input.unsqueeze(1).expand_as(self.G_real_D),
                                                          self.opt.CTOrder_Xray1).detach()
