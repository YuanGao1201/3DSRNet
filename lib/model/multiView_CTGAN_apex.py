...
class CTGAN(Base_Model):
    ...
    def backward_D_basic(self, D_network, input_real, input_fake, fake_pool, criterionGAN, loss_weight):
        ...
        with amp.scale_loss(gan_loss, self.optimizer_D) as scaled_lossD:
    ...
    def backward_D(self):
        ...
        # im = tensor_to_image(im)
        # for i in range(im.shape[0]):
        #     image_name = 'real_input_D_' + str(i) + '.png'
        #     save_path = os.path.join('/home/gy/SE/X2CT/3DGAN/save_models/multiView_CTGAN/VerSe_apex/fake', image_name)
        #     save_image(im[i], save_path)
        # im = torch.squeeze(fake_input, dim=0).detach()
        # im = tensor_to_image(im)
        # for i in range(im.shape[0]):
        #     image_name = 'fake_input_D_' + str(i) + '.png'
        #     save_path = os.path.join('/home/gy/SE/X2CT/3DGAN/save_models/multiView_CTGAN/VerSe_apex/fake', image_name)
        #     save_image(im[i], save_path)
    ...
    def backward_G(self):
        ...
        # im = tensor_to_image(im)
        # for i in range(im.shape[0]):
        #     image_name = 'real_input_G_' + str(i) + '.png'
        #     save_path = os.path.join('/home/gy/SE/X2CT/3DGAN/save_models/multiView_CTGAN/VerSe_apex/fake', image_name)
        #     save_image(im[i], save_path)
        # im = torch.squeeze(fake_input, dim=0).detach()
        # im = tensor_to_image(im)
        # for i in range(im.shape[0]):
        #     image_name = 'fake_input_G_' + str(i) + '.png'
        #     save_path = os.path.join('/home/gy/SE/X2CT/3DGAN/save_models/multiView_CTGAN/VerSe_apex/fake', image_name)
        #     save_image(im[i], save_path)
        ...
        # apex lossG.backward()
        with amp.scale_loss(self.loss_total_G, self.optimizer_G) as scaled_lossG:
