...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.src_conv64 = nn.Conv3d(in_channels=64, out_channels=128, kernel_size=(1,1,1))
        self.src_conv32 = nn.Conv3d(in_channels=32, out_channels=128, kernel_size=(1,1,1))
    ...
    def forward(self, srcs, masks, pos_embeds):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            if c == 64:
                src = self.src_conv64(src)
            elif c == 32:
                src = self.src_conv32(src)
            spatial_shape = (d, h, w)
            spatial_shapes.append(spatial_shape)
            src = src.flatten(2).transpose(1, 2)
            # if not src.shape[2] == 128:
            #     src = src.repeat(1, 1, 128 // src.shape[2])
            mask = mask.flatten(1)
            pos_embed = pos_embed.flatten(2).transpose(1, 2)
            t = self.level_embed[lvl].view(1, 1, -1)
            lvl_pos_embed = pos_embed + t
        ...
        # bs, c, d, h, w = srcs.shape
        # spatial_shape = (d, h, w)
        # spatial_shapes = spatial_shape
        # src = srcs.flatten(2).transpose(1, 2)
